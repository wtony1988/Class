...
- (void)layoutSubviews
{
    ...
    if (self.numberOfSegments >= 2)
    {
        ...
        if ([self isRangeSlidersPlacedAtExtremePosition])
        {
            ...
            self.minRangeLabel.center = CGPointMake(CGRectGetMidX(self.startSliderButton.frame), CGRectGetMidY(self.bounds) + SLIDER_BUTTON_WIDTH - 15.0);
            self.maxRangeLabel.center = CGPointMake(CGRectGetMidX(self.endSliderButton.frame), CGRectGetMidY(self.bounds) + SLIDER_BUTTON_WIDTH - 15.0);
...
- (void)setDefaultValues
{
    ...
    self.rangeDisplayLabelColor = COLOR_CLASS_DARK_CYAN;
    
    self.segmentSelectedColor = [UIColor colorWithRed:0.9 green:0.9 blue:0.9 alpha:1.0];
    self.segmentUnSelectedColor = [UIColor lightGrayColor];
    ...
    if ([self.startSliderButton isSelected])
    {
        if ([self shouldStartButtonSlideForPoint:point])
        {
            [UIView animateWithDuration:0.1 animations:^{
                ...
                self.minRangeLabel.center = CGPointMake(CGRectGetMidX(self.startSliderButton.frame), CGRectGetMaxY(self.startSliderButton.frame) + SLIDER_BUTTON_WIDTH / 2 - 15.0);
    ...
    else if ([self.endSliderButton isSelected])
    {
        if ([self shouldEndButtonSlideForPoint:point])
        {
            [UIView animateWithDuration:0.1 animations:^{
                ...
                self.maxRangeLabel.center = CGPointMake(CGRectGetMidX(self.endSliderButton.frame), CGRectGetMaxY(self.endSliderButton.frame) + SLIDER_BUTTON_WIDTH / 2 - 15.0);
